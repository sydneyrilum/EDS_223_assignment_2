...
# dissolve buffers
highways_buffer = st_union(highways_buffer)
```
...
```{r}
houses <- st_read("data/gis_osm_buildings_a_free_1.gpkg", query = query2)
```
...
cropped_blackout_mask_wo_highways = st_difference(cropped_blackout_mask, highways_buffer)
